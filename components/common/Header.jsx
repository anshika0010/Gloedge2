...
const navLink =[
  {name:"Eligibility Check",link:"eligibility",color:"after:bg-red-500 hover:bg-red-600 hover:text-white"},
  {name:"Skilled Migration",link:"migrate",color:"after:bg-[#6e298e] hover:bg-[#6e298e] hover:text-white"},
  {name:"Study",link:"study",color:"after:bg-yellow-500 hover:bg-yellow-500 hover:text-white"},
  {name:"Job",link:"job",color:"after:bg-[#009bad] hover:bg-[#009bad] hover:text-white"},
  {name:"Business & Investment",link:"visa",color:"after:bg-[#f55f3e] hover:bg-[#f55f3e] hover:text-white"},
  // {name:"Citizenship",link:"invest",color:"after:bg-green-500 hover:bg-green-500 hover:text-white"},
  {name:"Language Training",link:"language-training",color:"after:bg-blue-700 hover:bg-blue-700 hover:text-white"}
]
...
<Swiper
    ...
    >
      <SwiperSlide>
        <div className="h-full flex flex-col justify-center">
          <h1 className="font-semibold text-sm">August 23,2025 | Kazakhstan</h1>
          <p className="text-xs ">Kazakhstan : Investor visa Introduced</p>
        </div>
      </SwiperSlide>
      <SwiperSlide>
        <div className="h-full flex flex-col justify-center">
      ...
      <SwiperSlide>
        ...
        <div>
          <button className="bg-[#da010b] text-white font-semibold py-2 px-4 rounded-sm shadow-md hover:shadow-xl transition-all duration-300">Watch Now</button>
        </div>
        </div>
      </div>
    <div className="flex container mx-auto px-4 md:px-0 gap-3 items-center flex justify-between ">
        ...
        <div className="flex  ">
          <p className="flex gap-1 items-center font-semibold text-gray-500 mr-4"><MdOutlineMailOutline className=""/> www.gloedgevisas.com</p>
        <p className="flex gap-1 items-center font-semibold text-gray-500 mr-4"><FaWhatsapp style={{color:"var(--color-green-700)"}}/> +91 8878-8878-41</p>
        <Link href={`/${language}/affiliate`} className={`
                      hover:bg-[#494E3C] hover:text-white
                      text-gray-600 font-semibold navitem-top  relative `} >
                Affiliate
              </Link>
        <Link href={`/${language}/blogs`} className={`
        ...
        <div>
          <button className="hover:bg-[#da010b] hover:text-white
                      text-gray-600 font-semibold navitem-top  relative border-r border-gray-300">Pay Now</button>
        </div>
        </div>
        ...
        <div className="container mx-auto px-4 md:px-0">
          <div className="flex justify-between items-center h-14">
            ...
            <Link href="/" className="relative">
              <Image src={'/logo1 (2).png'} alt="logo"
              width={500}
              height={500} className="w-auto h-[40px] md:h-[55px] " />
            </Link>
            ...
            >

              {
                navLink.map((item,index)=>{
                  return(
                    <Link key={index} href={`/${language}/${item?.link}`} className={`
                      ${item.color}
                      nav-link  text-gray-600 font-semibold navitem  relative ${navLink.length-1===index?"border-r border-gray-300":"" }`} >
                {item?.name}
              </Link>
                  )
                })
   ...
   ></div>
      ...
      >
        ...
        <p onClick={()=>redirect_Page("/")} className="text-sm border-b border-zinc-200 py-2 px-4 text-gray-600 nav-link">
              Home
            </p>
          <div className="relative group">
      {/* Dropdown Button */}
      ...
      >
        <div className="py-1">
                  <p onClick={()=>redirect_Page("/about-us/team")}
                    className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                  >
                    Our Author
                  </p>
...
{/* {
              ...
              <p onClick={()=>redirect_Page(`/books`)} className="text-sm border-b border-zinc-200 py-2 px-4 text-gray-600">
                Books
              </p>
              <p onClick={()=>redirect_Page(`/news-and-events`)}
                className="text-sm border-b border-zinc-200 py-2 px-4 text-gray-600"
              >
                News and Events
              </p>
               <p onClick={()=>redirect_Page(`/contact-us`)}
                className="text-sm border-b border-zinc-200 py-2 px-4 text-gray-600"
              >
                Contact Us
              </p>
